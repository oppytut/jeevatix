...
## Pre-Launch Hardening — 2026-05-30

A pre-launch hardening pass extended the baseline above with three pieces of
defense in depth that the original review did not require but that became
relevant once the buyer, seller, and admin portals approached production:

### Browser security headers

`apps/{buyer,seller,admin}/src/lib/security-headers.ts` adds a shared
SvelteKit `Handle` that sets the following on every portal response:

- `X-Frame-Options: DENY`
- `X-Content-Type-Options: nosniff`
- `Referrer-Policy: strict-origin-when-cross-origin`
- `Permissions-Policy: accelerometer=(), camera=(self), display-capture=(),
geolocation=(), gyroscope=(), magnetometer=(), microphone=(), midi=(),
payment=(), usb=(), xr-spatial-tracking=()`
- `Cross-Origin-Opener-Policy: same-origin`
- `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`
  (only when `APP_ENVIRONMENT=production`)

Camera permission is allow-list `self` so the seller QR check-in flow keeps
working. Every header is set with `headers.has()` guards so a downstream
handler can override without conflict.

### Content Security Policy

CSP is configured via SvelteKit's `kit.csp` block in each
`svelte.config.js` so the framework can inject per-request nonces into
`<script>` and `<style>` tags. The directive set is:

```
default-src   'self'
script-src    'self' 'wasm-unsafe-eval'      (+ per-request nonce from kit.csp)
style-src     'self' 'unsafe-inline'         (+ per-request nonce from kit.csp)
img-src       'self' https: data: blob:
font-src      'self' data:
connect-src   'self' https://*.sentry.io https://*.ingest.sentry.io
              + buyer-only: wss://${PUBLIC_PARTYKIT_HOST} https://${PUBLIC_PARTYKIT_HOST}
                            (or wss: ws: when PUBLIC_PARTYKIT_HOST is empty)
...
emits inline `style=` attributes that nonce-injection cannot reach. SvelteKit
auto-mode adds the per-request nonce to `script-src` and `style-src` on top
of the configured directives, so inline `<script>` and `<style>` tags
generated by the framework continue to work without `'unsafe-inline'` on
scripts.

`'wasm-unsafe-eval'` is retained on `script-src` only to keep the door open
for future WASM modules; nothing in the current build relies on it.

### Rate limit coverage extension

The shared Durable Object backed limiter was already in place for `/auth/login`,
`/auth/register`, `/auth/register/seller`, and `POST /reservations`. The
hardening pass added six new factories in `apps/api/src/middleware/rate-limit.ts`
and applied them across the remaining mutation surface:

| Factory                                    | Limit       | Key             | Applied to                                                                                                   |
| ------------------------------------------ | ----------- | --------------- | ------------------------------------------------------------------------------------------------------------ |
| `sensitiveAuthRateLimitMiddleware`         | 5/min/IP    | client IP       | `/auth/forgot-password`, `/auth/reset-password`, `/auth/logout`                                              |
| `verifyEmailRateLimitMiddleware`           | 10/min/IP   | client IP       | `/auth/verify-email` (GET link + POST body)                                                                  |
| `paymentWebhookRateLimitMiddleware`        | 30/min/IP   | client IP       | `/webhooks/payment`                                                                                          |
| `authenticatedMutationRateLimitMiddleware` | 30/min/user | user id (or IP) | orders, users `PATCH /me*`, payment initiation, seller events/tiers/profile/checkin, reservation cancel      |
| `adminMutationRateLimitMiddleware`         | 60/min/user | user id (or IP) | every admin POST/PUT/PATCH/DELETE under categories, events, orders, payments, users, notifications/broadcast |
| `uploadRateLimitMiddleware`                | 20/min/user | user id (or IP) | `POST /upload`                                                                                               |

All factories fall back to client IP when `c.var.user` is unset (e.g., the
unauthenticated payment webhook caller) and reuse the existing Durable Object
counter with the in-memory fallback used in local/test contexts.

### Input validation hardening

`GET /auth/verify-email` previously read the `token` query parameter via
`URL.searchParams.get(...)` directly. The handler now validates with
`verifyEmailQuerySchema` (`z.string().min(1).max(4096)`) using
`safeParse`, matching the JSON variant. Oversized tokens are rejected
before reaching the JWT verifier.

The post-2026-05-30 audit confirmed every other route handler already
consumes user input through `c.req.valid('json' | 'query' | 'param')`
backed by zod schemas. The only remaining unstructured input is the
multipart body on `POST /upload`, which is validated at service level
(MIME type + file size).
...
```bash
curl -I https://buyer.jeevatix.my.id/
curl -I https://seller.jeevatix.my.id/
curl -I https://admin.jeevatix.my.id/
```

Expected on every response:
