// Tier 3 — Verifies multi-tenant isolation across roles. Seller A cannot
// read or mutate Seller B's events/orders, buyer cannot read another
// buyer's order, and unauthenticated/cross-role access is rejected.
...
test.describe('Tier 3 — multi-tenant isolation across roles', () => {
  ...
  test("seller A cannot read or edit seller B's draft event via /seller/events/:id", async ({
    request,
  }) => {
    ...
    const crossDeleteResponse = await request.delete(
      `${API_URL}/seller/events/${sellerAEvent.id}`,
      {
        headers: {
          Authorization: `Bearer ${sellerBSession.access_token}`,
          Accept: 'application/json',
    ...
    expect(crossDeleteResponse.ok()).toBe(false);
    expect([403, 404]).toContain(crossDeleteResponse.status());

    const crossSubmitResponse = await request.post(
      `${API_URL}/seller/events/${sellerAEvent.id}/submit`,
      {
        headers: {
          Authorization: `Bearer ${sellerBSession.access_token}`,
          Accept: 'application/json',
    ...
    expect(crossSubmitResponse.ok()).toBe(false);
    expect([403, 404]).toContain(crossSubmitResponse.status());
  ...
  test('cross-role access is rejected (buyer→admin, buyer→seller, seller→buyer, seller→admin)', async ({
    request,
  }) => {
  ...
  test('unauthenticated requests to scoped routes return 401/403', async ({ request }) => {
    const targets = [
