// Tier 3 — Verifies payment webhook idempotency and signature enforcement.
// Replays a successful webhook 5x and asserts the order stays confirmed exactly
// once with a single ticket issued. Also asserts bad/missing signatures 401
// without side-effects, and unknown external_ref returns 404.
...
test.describe('Tier 3 — payment webhook idempotency & signature enforcement', () => {
  ...
  test('replaying success webhook 5x keeps order confirmed exactly once', async ({ request }) => {
  ...
  test('webhook with invalid or missing signature returns 401 and order stays pending', async ({
    request,
  }) => {
    const eventFixture = await withRetry(() => createPublishedEventFixture(request));
  ...
  test('webhook with unknown external_ref returns 404 with no side-effects', async ({
    request,
  }) => {
