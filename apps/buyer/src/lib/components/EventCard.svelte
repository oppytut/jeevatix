...
>
  <div class={cn('relative overflow-hidden', compact ? 'aspect-[4/3]' : 'aspect-[16/10]')}>
    ...
    <div class="absolute left-4 top-4 flex flex-wrap gap-2">
      {#if event.is_featured}
        <span class="rounded-full border border-white/30 bg-white/15 px-3 py-1 text-[11px] font-semibold tracking-[0.22em] text-white uppercase backdrop-blur">
          Featured
        </span>
      {/if}
      <span class="rounded-full border border-white/30 bg-slate-950/30 px-3 py-1 text-[11px] font-semibold tracking-[0.2em] text-white uppercase backdrop-blur">
        {event.status === 'ongoing' ? 'Ongoing' : 'Published'}
      </span>
    </div>

    <div class="absolute inset-x-0 bottom-0 p-4 text-white sm:p-5">
      <p class="text-xs font-semibold tracking-[0.26em] uppercase text-white/70">{event.venue_city}</p>
      <h3 class="mt-2 line-clamp-2 text-xl font-semibold tracking-tight sm:text-2xl">{event.title}</h3>
    </div>
  </div>
  ...
  <div class="space-y-4 p-5 sm:p-6">
    ...
    <p class="line-clamp-2 text-sm leading-6 text-slate-600">
      {event.description ?? `Event di ${event.venue_name} dengan akses tiket hingga ${event.max_tickets_per_order} per order.`}
    </p>
    ...
    <div class="flex items-end justify-between gap-4 border-t border-slate-100 pt-4">
      <div>
        ...
        <p class="mt-1 text-lg font-semibold text-slate-950 sm:text-xl">{formatCurrency(event.min_price)}</p>
      </div>

      <span class="inline-flex items-center gap-2 text-sm font-medium text-slate-700 transition group-hover:text-slate-950">
