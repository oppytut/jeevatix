...
>
  <div class={cn('relative overflow-hidden', compact ? 'aspect-[4/3]' : 'aspect-[16/10]')}>
    ...
    <div class="absolute top-4 left-4 flex flex-wrap gap-2">
      ...
      {/if}
      <span
        class="bg-foreground/30 rounded-full border border-white/30 px-3 py-1 text-[11px] font-semibold tracking-[0.2em] text-white uppercase backdrop-blur"
      >
  ...
  <div class="space-y-4 p-5 sm:p-6">
    <div class="text-muted-foreground flex flex-wrap gap-3 text-sm">
      <span class="bg-muted inline-flex items-center gap-2 rounded-full px-3 py-1.5">
