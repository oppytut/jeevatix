...
// via custom domain due to Cloudflare same-zone 522 error. Configurable via
// INTERNAL_API_URL env var (injected through SST Worker environment) so
// production can point at the production workers.dev URL without a source
// change. Falls back to the staging workers.dev URL when the var is unset.
//
