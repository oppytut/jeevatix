...
<section class="mx-auto flex min-h-[calc(100vh-10rem)] max-w-3xl items-center py-6 lg:py-10">
  <Card class="w-full rounded-[2rem] border border-white/80 bg-white/90 p-7 shadow-[0_26px_80px_rgba(15,23,42,0.10)] sm:p-9">
    <div class="flex items-start justify-between gap-4">
      <div class="space-y-2">
        <p class="text-sm font-semibold tracking-[0.3em] text-slate-500 uppercase">Reset Password</p>
    ...
    {#if !data.token}
      <div class="mt-8 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-4 text-sm text-rose-700">
        Token reset password tidak ditemukan. Gunakan link reset yang valid dari email atau halaman forgot password.
      </div>
    {:else}
      <form class="mt-8 space-y-5" method="POST">
        ...
        <div class="space-y-2">
          <label class="text-sm font-medium text-slate-700" for="confirm_password">Konfirmasi Password Baru</label>
          <Input
            id="confirm_password"
    ...
    {#snippet footer()}
      <div class="flex items-center justify-between gap-4 text-sm text-slate-500">
        ...
        <a class="font-medium text-sky-700 hover:text-sky-800" href="/login">Kembali ke login</a>
      </div>
    {/snippet}
  </Card>
</section>