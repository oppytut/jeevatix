...
<svelte:head>
  ...
  <meta name="description" content={data.event.description ?? `Detail event ${data.event.title} di Jeevatix.`} />
</svelte:head>

<section class="space-y-8 py-6 sm:py-8 lg:py-10">
  <div class="overflow-hidden rounded-[2.5rem] border border-white/80 bg-white/85 shadow-[0_28px_90px_rgba(15,23,42,0.08)]">
    <div class="relative aspect-16/8 overflow-hidden bg-[linear-gradient(135deg,#f97316,#facc15_45%,#0ea5e9)]">
      {#if data.event.banner_url}
        <img src={data.event.banner_url} alt={data.event.title} class="h-full w-full object-cover" />
      {/if}
      <div class="absolute inset-0 bg-linear-to-t from-slate-950/75 via-slate-950/10 to-transparent"></div>

      <div class="absolute inset-x-0 bottom-0 p-6 text-white sm:p-8 lg:p-10">
        <div class="flex flex-wrap gap-2">
          {#each data.event.categories as category}
            <a href={`/categories/${category.slug}`} class="rounded-full border border-white/25 bg-white/10 px-3 py-1 text-xs font-semibold tracking-[0.22em] uppercase backdrop-blur">
              {category.name}
            </a>
          {/each}
        </div>
        <h1 class="mt-4 max-w-4xl text-4xl font-semibold tracking-tight sm:text-5xl lg:text-6xl">{data.event.title}</h1>
  ...
  <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
    <div class="space-y-6">
      {#if data.event.images.length > 0}
        <section class="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:p-6">
          <div class="flex items-center justify-between gap-4">
            <div>
              <p class="text-sm font-semibold tracking-[0.26em] text-slate-500 uppercase">Gallery</p>
              <h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Preview suasana event</h2>
            </div>
          </div>
          <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {#each data.event.images as image}
              <div class="overflow-hidden rounded-[1.5rem] bg-slate-100">
                <img src={image.image_url} alt={data.event.title} class="aspect-4/3 h-full w-full object-cover" />
              </div>
            {/each}
          </div>
        </section>
      ...
      <section class="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:p-6">
        <p class="text-sm font-semibold tracking-[0.26em] text-slate-500 uppercase">Tentang Event</p>
      ...
      <section class="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:p-6">
        <p class="text-sm font-semibold tracking-[0.26em] text-slate-500 uppercase">Lokasi & Jadwal</p>
          ...
          <div class="space-y-4 rounded-[1.5rem] bg-slate-50 p-5">
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p class="mt-2 text-sm leading-7 text-slate-600">{formatEventDateRange(data.event.start_at, data.event.end_at)}</p>
              </div>
            </div>
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p class="mt-2 text-sm leading-7 text-slate-600">
                  Dimulai {formatLongDateTime(data.event.sale_start_at)} dan ditutup {formatLongDateTime(data.event.sale_end_at)}.
                </p>
              </div>
            </div>
          </div>
    ...
    <aside class="space-y-6">
      <section class="rounded-[2rem] border border-white/80 bg-white/92 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-6">
        <div class="flex items-start justify-between gap-4">
          <div>
            <p class="text-sm font-semibold tracking-[0.26em] text-slate-500 uppercase">Ticket Tiers</p>
            <h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Pilih tier yang paling cocok</h2>
          </div>
          ...
          {#each data.event.tiers as tier}
            <div class="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4">
              ...
              <div class="mt-4 flex items-end justify-between gap-4">
                <div>
                  <p class="text-xs font-semibold tracking-[0.22em] text-slate-500 uppercase">Harga</p>
                  <p class="mt-1 text-xl font-semibold text-slate-950">{formatCurrency(tier.price)}</p>
                </div>
                <div class="text-right text-sm text-slate-600">
                  <p>Sisa {tier.remaining} dari {tier.quota}</p>
      ...
      <section class="rounded-[2rem] border border-white/80 bg-white/92 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-6">
        <p class="text-sm font-semibold tracking-[0.26em] text-slate-500 uppercase">Penyelenggara</p>
        <div class="mt-5 flex items-start gap-4">
          <div class="flex size-16 items-center justify-center overflow-hidden rounded-2xl bg-slate-100 text-lg font-semibold text-slate-700">
            {#if data.event.seller.logo_url}
              <img src={data.event.seller.logo_url} alt={data.event.seller.org_name} class="h-full w-full object-cover" />
            {:else}
          ...
          <div>
            ...
            <p class="mt-2 text-sm leading-7 text-slate-600">
              {data.event.seller.org_description ?? 'Penyelenggara belum menambahkan deskripsi organisasi.'}
            </p>
          </div>
        </div>
      </section>
