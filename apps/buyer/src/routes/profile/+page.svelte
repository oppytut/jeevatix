...
<section class="space-y-8 py-6 sm:py-8 lg:py-10">
  <div class="rounded-[2.5rem] border border-white/80 bg-[linear-gradient(135deg,#fff8ef_0%,#fff2df_42%,#edf8ff_100%)] p-7 shadow-[0_26px_90px_rgba(15,23,42,0.08)] sm:p-9">
    <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
      <div class="space-y-3">
        ...
        <p class="max-w-3xl text-base leading-7 text-slate-600 sm:text-lg">
          Perbarui identitas dasar akun Anda, simpan avatar yang mudah dikenali, dan pastikan password tetap aman.
        </p>
      </div>
    ...
    <Card class="rounded-[2rem] border border-white/80 bg-white/92 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] sm:p-7">
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Identitas akun buyer</h2>
      ...
      <form class="mt-8 space-y-6" method="POST" action="?/updateProfile" enctype="multipart/form-data">
        <div class="flex flex-col gap-5 rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5 sm:flex-row sm:items-center">
          <div class="flex size-24 items-center justify-center overflow-hidden rounded-[1.5rem] bg-[linear-gradient(135deg,#f97316,#facc15)] text-2xl font-semibold text-slate-950">
            {#if getAvatarSrc()}
              <img src={getAvatarSrc() ?? undefined} alt={getProfile().full_name} class="h-full w-full object-cover" />
            {:else}
          ...
          <div class="min-w-0 flex-1 space-y-3">
            ...
            <label class="inline-flex w-fit cursor-pointer items-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:text-slate-950" for="avatar">
              <Camera class="size-4" />
              Pilih Avatar
            </label>
            <input id="avatar" name="avatar" type="file" accept="image/jpeg,image/png,image/webp" class="hidden" onchange={handleAvatarChange} />
        ...
        <div class="grid gap-5 sm:grid-cols-2">
          <div class="space-y-2 sm:col-span-2">
            ...
            <Input id="full_name" name="full_name" value={getProfile().full_name} placeholder="Nama lengkap buyer" required />
          </div>

          <div class="space-y-2">
            <label class="text-sm font-medium text-slate-700" for="email">Email</label>
            <div class="relative">
              <Mail class="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-slate-400" />
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              <Phone class="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-slate-400" />
              <Input id="phone" name="phone" value={getProfile().phone ?? ''} class="pl-10" placeholder="08xxxxxxxxxx" />
            </div>
          </div>
        </div>
    ...
    <Card class="rounded-[2rem] border border-white/80 bg-white/92 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] sm:p-7">
      ...
      <form class="mt-8 space-y-5" method="POST" action="?/changePassword">
        <div class="space-y-2">
          ...
          <Input id="old_password" name="old_password" type="password" autocomplete="current-password" placeholder="Masukkan password saat ini" required />
        </div>

        <div class="space-y-2">
          <label class="text-sm font-medium text-slate-700" for="new_password">New Password</label>
          <Input id="new_password" name="new_password" type="password" autocomplete="new-password" placeholder="Minimal 8 karakter" required />
        </div>

        <div class="space-y-2">
          <label class="text-sm font-medium text-slate-700" for="confirm_password">Confirm New Password</label>
          <Input id="confirm_password" name="confirm_password" type="password" autocomplete="new-password" placeholder="Ulangi password baru" required />
        </div>
