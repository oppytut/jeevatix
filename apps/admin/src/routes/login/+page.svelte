<script lang="ts">
  import { enhance } from '$app/forms';
  import { ShieldCheck, TriangleAlert, Loader2 } from '@lucide/svelte';
  import { Button, Card, Input } from '@jeevatix/ui';

  let { form }: import('./$types').PageProps = $props();
  let submitting = $state(false);
</script>

<svelte:head>
  <title>Admin Login | Jeevatix</title>
  <meta
    name="description"
    content="Masuk ke admin console Jeevatix untuk mengelola pengguna, kategori, transaksi, dan seller."
  />
</svelte:head>

<section
  class="border-border bg-card relative overflow-hidden rounded-[2rem] border shadow-[0_30px_90px_rgba(15,23,42,0.12)]"
>
  <div
    class="absolute inset-x-0 top-0 h-40 opacity-70"
    style="background-image: var(--gradient-overlay-top);"
  ></div>

  <div class="relative grid min-h-[calc(100vh-4rem)] gap-0 lg:grid-cols-[1.05fr_0.95fr]">
    <div
      class="bg-foreground text-background flex flex-col justify-between px-8 py-10 sm:px-10 lg:px-12 lg:py-12"
    >
      <div class="space-y-6">
        <div
          class="bg-card/5 inline-flex w-fit items-center gap-3 rounded-full border border-white/10 px-4 py-2"
        >
          <span class="size-2.5 rounded-full bg-emerald-400"></span>
          <span class="text-xs font-semibold tracking-[0.35em] uppercase">Jeevatix Admin</span>
        </div>

        <div class="space-y-4">
          <h1 class="max-w-lg text-4xl font-semibold tracking-tight sm:text-5xl">
            Kontrol operasional platform dari satu ruang kerja yang ketat.
          </h1>
          <p class="text-background/80 max-w-xl text-base leading-7 sm:text-lg">
            Login admin dipisahkan dari portal buyer dan seller untuk menjaga review event, moderasi
            akun, serta monitoring transaksi tetap terpusat dan aman.
          </p>
        </div>
      </div>

      <div class="grid gap-4 sm:grid-cols-3">
        <div class="bg-card/5 rounded-[1.5rem] border border-white/10 p-4">
          <p class="text-muted-foreground/70 text-xs font-semibold tracking-[0.25em] uppercase">
            Users
          </p>
          <p class="mt-3 text-3xl font-semibold">A3</p>
          <p class="text-background/80 mt-2 text-sm">Moderasi akun buyer, seller, dan admin.</p>
        </div>
        <div class="bg-card/5 rounded-[1.5rem] border border-white/10 p-4">
          <p class="text-muted-foreground/70 text-xs font-semibold tracking-[0.25em] uppercase">
            Catalog
          </p>
          <p class="mt-3 text-3xl font-semibold">A13</p>
          <p class="text-background/80 mt-2 text-sm">Kelola kategori event dan kurasi listing.</p>
        </div>
        <div class="bg-card/5 rounded-[1.5rem] border border-white/10 p-4">
          <p class="text-muted-foreground/70 text-xs font-semibold tracking-[0.25em] uppercase">
            Payments
          </p>
          <p class="mt-3 text-3xl font-semibold">A11</p>
          <p class="text-background/80 mt-2 text-sm">Pantau order, pembayaran, dan reservasi.</p>
        </div>
      </div>
    </div>

    <div
      class="flex items-center justify-center bg-[var(--gradient-section-alt)] px-6 py-10 sm:px-8 lg:px-12"
    >
      <Card
        title={undefined}
        description={undefined}
        class="border-border bg-card/90 w-full max-w-xl rounded-[2rem] border p-7 shadow-[0_28px_80px_rgba(15,23,42,0.08)] sm:p-9"
      >
        <div class="flex items-start justify-between gap-4">
          <div class="space-y-2">
            <p class="text-muted-foreground text-sm font-semibold tracking-[0.32em] uppercase">
              Sign In
            </p>
            <h2 class="text-foreground text-3xl font-semibold tracking-tight">
              Masuk ke admin console
            </h2>
            <p class="text-muted-foreground text-sm leading-6">
              Gunakan akun admin yang memiliki akses ke dashboard internal Jeevatix.
            </p>
          </div>

          <div
            class="flex size-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-700"
          >
            <ShieldCheck class="size-7" />
          </div>
        </div>

        <form
          class="mt-8 space-y-5"
          method="POST"
          use:enhance={() => {
            submitting = true;
            return async ({ update }) => {
              submitting = false;
              await update();
            };
          }}
        >
          <div class="space-y-2">
            <label class="text-foreground text-sm font-medium" for="email">Email</label>
            <Input
              id="email"
              name="email"
              type="email"
              placeholder="admin@jeevatix.id"
              value={form?.values?.email ?? ''}
              autocomplete="email"
              required
            />
          </div>

          <div class="space-y-2">
            <label class="text-foreground text-sm font-medium" for="password">Password</label>
            <Input
              id="password"
              name="password"
              type="password"
              placeholder="Masukkan password"
              autocomplete="current-password"
              required
            />
          </div>

          {#if form?.error}
            <div
              class="flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700"
            >
              <TriangleAlert class="mt-0.5 size-4 shrink-0" />
              <p>{form.error}</p>
            </div>
          {/if}

          <Button class="w-full" type="submit" disabled={submitting}>
            {#if submitting}
              <Loader2 class="mr-2 size-4 animate-spin" />
              Memproses...
            {:else}
              Login
            {/if}
          </Button>
        </form>

        {#snippet footer()}
          <div class="text-muted-foreground flex items-center justify-between gap-4 text-sm">
            <p>Portal ini khusus untuk administrator Jeevatix.</p>
            <p class="text-foreground font-medium">localhost:4302</p>
          </div>
        {/snippet}
      </Card>
    </div>
  </div>
</section>
